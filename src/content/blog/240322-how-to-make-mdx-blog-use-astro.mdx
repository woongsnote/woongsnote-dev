---
title: Astro로 구현하는 MDX 블로그
description: Astro와 Tailwind CSS를 사용해서 블로그를 구현하는 방법
...
9. 프로젝트가 문제 없이 생성되었는지 확인하기 위해, `VS Code`에서 방금 만든 폴더를 열고, 아래 명령어 실행(터미널에서 바로 Vs Cdoe 실행 명령어: `code .`)

```bash title="run dev server"
npm run dev
```

정상적으로 실행되면, 스타일링을 위해 `Tailwind CSS`를 설치한다.

## 2. Tailwind CSS 추가

아래 명령어를 사용해서 Tailwind CSS를 추가한다. 해당 명령어는 Astro에서 제공해주는 Integration을 이용해서 설치하는 방법이다.

```bash title="install tailwindcss"
...
```bash title="npm install tailwindcss"
...
※ Astro는 `className`이 아닌 `class`임을 주의해야 한다.
...
```bash title="install rehype-pretty-code"
