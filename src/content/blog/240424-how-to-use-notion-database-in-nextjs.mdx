...
- @notionhq/client: Notion API 사용을 위해 설치
- @notion-render/client: 가져온 데이터 렌더링을 위해 설치

```bash title="install packages"
npm install @notionhq/client @notion-render/client
```

`utils` 폴더를 생성하고, 그 안에 `notion.ts` 파일을 생성한다.
...
이제 노션에서 글을 작성하면, 다시 배포를 진행하지 않아도, 게시글이 자동으로 업데이트 될 것이다.
