...
description: 'Next.js와 Contentlayer를 사용한 블로그 템플릿 구현 후기'
pubDate: '2023-08-10'
...
- ContentLayer를 사용하기 위해, `next.config.js` , `tsconfig.json` , 수정이 필요하다.
...
게시글을 날짜순으로 보여주기 위해 아래와 같이 작성했다.
...
이제 제목만 보여주는 대신 각 포스트를 보여줄 컴포넌트로 만들어서 좀 더 꾸며보도록 하자.
...
아직, 상세 페이지에 대한 코드를 작성하지 않아서, 각 Post를 클릭하면 **404 페이지**를 만나게 된다.

이를 해결하기 위해 **상세 페이지**를 만들어준다.
...
※ app 디렉토리에 `not-found.tsx` 파일로 404 페이지를 직접 구현할 수도 있다.
